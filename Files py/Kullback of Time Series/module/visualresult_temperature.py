...
class TransformData:
    ...
    @staticmethod
    def create_list(data: pd.DataFrame, date_start: str, date_end: str, fiend: str) -> (list, list):
        print(data)
        ...
        while j + 1 < len(list_dist):
            print(list_dist[j]["Temperature"])
            #print(list_dist[j][fiend])
            p = TransformData.kl(list_dist[j][fiend], list_dist[j + 1][fiend])
            mi = list_dist[j][fiend].index.min().month
            ...
            if p == np.inf:
                ...
                ax = list_dist[j]['Temperature'].plot(kind='bar', x=list_dist[j].index,
                ...
                ax2 = list_dist[j+1]['Temperature'].plot(kind='bar', x=list_dist[j+1].index,
